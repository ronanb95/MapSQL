...
public class Table {
	...
	public void update(String[] columns, String[] values, Condition where) throws SQLException {
		//Follows similar set of steps to insert method, for each and then check if satisfies the condition
		for (Row row : rows) {
			if (row.satisfies(where, description)) {
				...
				//pass
				//throw new SQLException ("No rows satisfy where clause");
			}
		}
	...
	public void delete(Condition where) throws SQLException {
		...
		//Start at first node
		Position<Row> currentNode = rows.first();
		
		//Loop and del
		for (Row row: rows) {
			if (row.satisfies(where, description)) {
				rows.remove(currentNode);
				//throw new SQLException("Out of range delete??");
				break;
			}
			else {
				//Move to next node
				currentNode = rows.next(currentNode);
			}
		}
	}
